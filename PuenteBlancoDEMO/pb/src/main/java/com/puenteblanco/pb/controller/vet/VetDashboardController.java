...
public class VetDashboardController {
    ...
    public ResponseEntity<?> getAppointmentsByMonth(@RequestParam("ym") String ym) {
        ...
        Map<String, Integer> resultado = citas.stream()
                .collect(Collectors.groupingBy(
                        cita -> String.format("%02d", cita.getFecha().getDayOfMonth()),
    ...
    private Veterinario getCurrentVeterinario() {
        ...
        if (vet == null)
            throw new RuntimeException("Usuario no tiene veterinario asignado");
